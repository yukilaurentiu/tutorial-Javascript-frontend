...
// each of them to the console, along with the number of goals that were scored in
...
for (let i = 0; i < scoredPlayer.length; i++) {
  console.log(`Goal ${[i + 1]}: ${scoredPlayer[i]}`);
}

// for(const [i, player] of game.scored.entries())
//   console.log(`Goal ${i + 1}: ${player[i]}`);
...
average = Math.trunc(average / odds.length);
console.log('Odds avarage is', average)

// const ok = Object.keys(game.odds);
// for(let x of ok) {
//   console.log(`Odd of ${game[x]}: ${game.odds[x]}`)
// }
...
console.log(`An event happened, on average, every ${eventTime/gameEvents.size} minutes`)

for(let i = 0; i < 90; i = i + 9){
  for(const [key, value] of gameEvents){
    if(i >= key && key > (i - 9)){
      console.log(`An event happened, on average, every 9 minutes', ${i}: ${key}: ${value}`)
    }
  }
}
